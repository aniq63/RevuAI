...
function detectActivePlayStoreApp() {
  if (!chrome.tabs || !chrome.tabs.query) {
    $('app-detection-status').textContent = 'Automatic detection unavailable; paste the link manually';
    return;
  }
  chrome.tabs.query({ active: true, lastFocusedWindow: true }, tabs => {
    if (chrome.runtime.lastError) {
      $('app-detection-status').textContent = 'Paste the app ID or Play Store link manually';
      return;
    }
    ...
    if (!appId) {
      $('app-detection-status').textContent = 'Open a Play Store app page or paste a link manually';
      return;
    }
    $('app-id-input').value = appId;
    $('app-detection-status').textContent = 'Automatically detected from the active Play Store tab';
  });
}
...
(async () => {
  try {
    ...
    if (d.status !== 'ok') $('backend-warn').classList.remove('hidden');
  } catch(e) { $('backend-warn').classList.remove('hidden'); }
})();
detectActivePlayStoreApp();
$('detect-app-button').addEventListener('click', detectActivePlayStoreApp);
...
async function analyzeApp({ appId, scrapingMethod, forceRefresh }) {
  const res = await fetch(API_BASE + '/inference/analyze', {
    method: 'POST',
